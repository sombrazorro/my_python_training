...
def seperate(dll):
    """ Performance bottleneck. This function called many time.
        the the time of percall seem to be higher"""
    m = dll.count()
...
def merge(left, right):
    """ Performance bottleneck. This function called many time however
        the time of percall seem to be lower."""
    result = DoubleLinkedList()
    while left.begin and right.begin:
        if left.begin.value <= right.begin.value:
            result.push(left.begin.value)
